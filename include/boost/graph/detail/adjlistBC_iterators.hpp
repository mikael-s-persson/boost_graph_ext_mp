...
class adjlistBC_viter_from_iter
 ...
 public:
  ...
  explicit adjlistBC_viter_from_iter(ContIterator aVIt = ContIterator())
      : v_it(aVIt){};
...
struct adjlistBC_select_vertex_iterator<poolBC, Container> {
  ...
  typedef filter_iterator<BC_is_not_hole<RawContainer>,
...
struct adjlistBC_select_out_edge_iterator<poolBC, Container, EDesc> {
  typedef typename Container::container_type RawContainer;
  typedef filter_iterator<BC_edge_is_not_hole<RawContainer>,
...
class adjlistBC_edge_iterator
 ...
 public:
  ...
  void increment() {
    ...
    while (true) {
      while (adjlistBC_check_desc_validity(
                 BC_desc_to_value(*p_cont, e.source).out_edges, e.edge_id) == 2)
        ++(e.edge_id);
      if (adjlistBC_check_desc_validity(
