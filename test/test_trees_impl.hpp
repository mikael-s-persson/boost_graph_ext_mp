...
typename enable_if<is_vertex_list_graph<TreeType>, void>::type
...
template <typename TreeType>
typename disable_if<is_vertex_list_graph<TreeType>, void>::type
intint_do_final_vertex_check(const TreeType&){};
...
template <typename TreeType>
typename disable_if<is_vertex_list_graph<TreeType>, void>::type
intint_check_tree_vertex_count(const TreeType&, std::size_t){};

template <typename TreeType>
typename enable_if<is_bidirectional_graph<TreeType>, void>::type
...
template <typename TreeType>
typename disable_if<is_bidirectional_graph<TreeType>, void>::type
intint_do_in_edge_check(const TreeType&,
                        typename graph_traits<TreeType>::vertex_descriptor, int,
                        int){};

template <typename TreeType>
typename enable_if<is_adjacency_matrix<TreeType>, void>::type
...
template <typename TreeType>
typename disable_if<is_adjacency_matrix<TreeType>, void>::type
intint_do_edge_check(const TreeType&,
                     typename graph_traits<TreeType>::vertex_descriptor,
                     typename graph_traits<TreeType>::vertex_descriptor, int,
                     int){};
...
BOOST_AUTO_TEST_CASE_TEMPLATE(intint_treetest, TreeType,
                              intint_treetest_types) {
  typedef typename graph_traits<TreeType>::vertex_descriptor Vertex;
  typedef typename graph_traits<TreeType>::edge_descriptor Edge;
  typedef typename graph_traits<TreeType>::out_edge_iterator OutEdgeIter;
  ...
  Edge e_rc[4];
  for (int i = 0; i < 4; ++i) {
    BOOST_CHECK_NO_THROW(tie(v_rc[i], e_rc[i]) =
  ...
  for (std::size_t i = 0; i < 4; ++i) {
    BOOST_CHECK_NO_THROW(tie(v_rc1c[i], e_rc1c[i]) = add_child_vertex(
                             v_rc[0], vp_rc1c[i], ep_rc1c[i], g));
  };
  ...
  BOOST_CHECK_EQUAL(g[v_root], 1);
  {
    ...
    for (; cvi != cvi_end; ++cvi) {
      if (g[*cvi] == 2) {
        ...
        for (; ei != ei_end; ++ei) {
          BOOST_CHECK_EQUAL(g[*ei],
                            (g[source(*ei, g)] * 1000 + g[target(*ei, g)]));
  ...
  for (std::size_t i = 0; i < 4; ++i) {
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
    BOOST_CHECK_NO_THROW(tie(v_rc2c[i], e_rc2c[i]) =
                             add_child_vertex(v_rc[1], std::move(vp_rc2c[i]),
                                              std::move(ep_rc2c[i]), g));
#else
    BOOST_CHECK_NO_THROW(tie(v_rc2c[i], e_rc2c[i]) = add_child_vertex(
                             v_rc[1], vp_rc2c[i], ep_rc2c[i], g));
#endif
  };
  ...
  /* MutablePropertyTree */
  BOOST_CHECK_NO_THROW(tie(v_rc[0], e_rc[0]) =
