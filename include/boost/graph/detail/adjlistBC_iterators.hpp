...
struct adjlistBC_traversal_tag : public virtual vertex_list_graph_tag,
...
class adjlistBC_viter_from_iter
    : public iterator_facade<
          adjlistBC_viter_from_iter<ContIterator>, const ContIterator,
          typename std::iterator_traits<ContIterator>::iterator_category> {
 public:
  using self = adjlistBC_viter_from_iter<ContIterator>;

  explicit adjlistBC_viter_from_iter(ContIterator aVIt) : v_it(aVIt) {}
  adjlistBC_viter_from_iter() : adjlistBC_viter_from_iter(ContIterator()) {}
...
struct adjlistBC_viter_from_index
    : public iterator_facade<adjlistBC_viter_from_index, const std::size_t,
                             std::random_access_iterator_tag> {
 public:
  using self = adjlistBC_viter_from_index;

  explicit adjlistBC_viter_from_index(std::size_t aVIt) : v_it(aVIt) {}
  adjlistBC_viter_from_index() : adjlistBC_viter_from_index(0) {}
...
struct adjlistBC_select_vertex_iterator {
  using type = adjlistBC_viter_from_iter<typename Container::iterator>;

  static std::pair<type, type> create_range(Container& cont) {
    return {type(cont.begin()), type(cont.end())};
  }
};

template <typename Container>
struct adjlistBC_select_vertex_iterator<vecBC, Container> {
  using type = adjlistBC_viter_from_index;

  static std::pair<type, type> create_range(Container& cont) {
    return {type(0), type(cont.size())};
  }
};

template <typename Container>
struct adjlistBC_select_vertex_iterator<poolBC, Container> {
  using RawContainer = typename Container::container_type;
  using type =
      filter_iterator<BC_is_not_hole<RawContainer>, adjlistBC_viter_from_index>;
...
class adjlistBC_oeiter_from_iter
    : public iterator_facade<adjlistBC_oeiter_from_iter<EDesc>, const EDesc,
                             std::bidirectional_iterator_tag> {
 public:
  using self = adjlistBC_oeiter_from_iter<EDesc>;

  explicit adjlistBC_oeiter_from_iter(EDesc aEdge) : e_it(std::move(aEdge)) {}
  adjlistBC_oeiter_from_iter() : adjlistBC_oeiter_from_iter(EDesc()) {}
  ...
  bool equal(const self& rhs) const {
    return (this->e_it.source == rhs.e_it.source) &&
           (this->e_it.edge_id == rhs.e_it.edge_id);
  }
...
class adjlistBC_oeiter_from_index
    : public iterator_facade<adjlistBC_oeiter_from_index<EDesc>, const EDesc,
                             std::random_access_iterator_tag> {
 public:
  using self = adjlistBC_oeiter_from_index<EDesc>;

  explicit adjlistBC_oeiter_from_index(EDesc aEdge) : e_it(std::move(aEdge)) {}
  adjlistBC_oeiter_from_index() : adjlistBC_oeiter_from_index(EDesc()) {}
  ...
  std::ptrdiff_t distance_to(const self& rhs) const {
    return rhs.e_it.edge_id - this->e_it.edge_id;
  }
...
class adjlistBC_undir_ioeiter
    : public iterator_facade<adjlistBC_undir_ioeiter<EDesc, IEIter, OEIter>,
                             EDesc, std::bidirectional_iterator_tag, EDesc> {
 public:
  using self = adjlistBC_undir_ioeiter<EDesc, IEIter, OEIter>;

  adjlistBC_undir_ioeiter()
      : view_as_out_edges(true),
        cur_ie(),
        ie_end(),
        oe_beg(),
        cur_oe(),
  ...
  adjlistBC_undir_ioeiter(bool aViewAsOutEdges, IEIter aCurIE, IEIter aIEEnd,
      ...
      : view_as_out_edges(aViewAsOutEdges),
        ...
        oe_beg(aOEBeg),
        cur_oe(aOEBeg),
        oe_end(std::move(aOEEnd)) {}

  adjlistBC_undir_ioeiter(bool aViewAsOutEdges, IEIter aIEEnd, OEIter aOEBeg,
                          OEIter aCurOE, OEIter aOEEnd)
      : view_as_out_edges(aViewAsOutEdges),
        cur_ie(aIEEnd),
        ie_end(aIEEnd),
        oe_beg(aOEBeg),
...
struct adjlistBC_select_out_edge_iterator {
  using type = adjlistBC_oeiter_from_iter<EDesc>;

  template <typename Vertex>
  static std::pair<type, type> create_range(Vertex u, Container& cont) {
    return std::pair<type, type>(type(EDesc(u, cont.begin())),
                                 type(EDesc(u, cont.end())));
  }
  template <typename Vertex>
  static std::pair<type, type> create_range(Vertex u, Container* cont) {
...
struct adjlistBC_select_out_edge_iterator<vecBC, Container, EDesc> {
  using type = adjlistBC_oeiter_from_index<EDesc>;

  template <typename Vertex>
  static std::pair<type, type> create_range(Vertex u, Container& cont) {
    return std::pair<type, type>(type(EDesc(u, 0)),
                                 type(EDesc(u, cont.size())));
  }
  template <typename Vertex>
  static std::pair<type, type> create_range(Vertex u, Container* cont) {
    return create_range(u, *cont);
...
struct adjlistBC_select_out_edge_iterator<poolBC, Container, EDesc> {
  using RawContainer = typename Container::container_type;
  using type = filter_iterator<BC_edge_is_not_hole<RawContainer>,
...
int adjlistBC_is_desc_at_begin(::boost::container::vector<ValueType>& cont,
                               Desc d) {
  if (d == 0) {
    return 1;
  }
...
template <typename ValueType, typename Desc>
int adjlistBC_check_desc_validity(::boost::container::vector<ValueType>& cont,
                                  Desc d) {
  if (d >= cont.size()) {
    return 1;
  }
...
class adjlistBC_edge_iterator
    : public iterator_facade<adjlistBC_edge_iterator<VContainer, EDesc>,
                             const EDesc, std::bidirectional_iterator_tag> {
 public:
  using self = adjlistBC_edge_iterator<VContainer, EDesc>;

  adjlistBC_edge_iterator() : p_cont(nullptr), e() {}

  static self begin(VContainer& vcont) {
    using RawEDesc = typename EDesc::edge_id_type;
    self result;
    result.p_cont = &vcont;
    result.e.source = BC_iterator_to_desc(vcont, BC_get_begin_iter(vcont));
    while (true) {
      while (adjlistBC_check_desc_validity(*result.p_cont, result.e.source) ==
             2) {
        ++(result.e.source);
      }
      if (adjlistBC_check_desc_validity(*result.p_cont, result.e.source) == 1) {
        result.e.edge_id =
            RawEDesc();  // end iterator (also begin iterator, vertex range is empty).
        break;
      }
      result.e.edge_id = BC_iterator_to_desc(
          BC_desc_to_value(*result.p_cont, result.e.source).out_edges,
          BC_get_begin_iter(
              BC_desc_to_value(*result.p_cont, result.e.source).out_edges));
      while (adjlistBC_check_desc_validity(
                 BC_desc_to_value(*result.p_cont, result.e.source).out_edges,
                 result.e.edge_id) == 2) {
        ++(result.e.edge_id);
      }
      if (adjlistBC_check_desc_validity(
              BC_desc_to_value(*result.p_cont, result.e.source).out_edges,
              result.e.edge_id) == 1) {
        ++(result.e.source);
        continue;
  ...
  void increment() {
    ...
    while (true) {
      while (adjlistBC_check_desc_validity(
                 BC_desc_to_value(*p_cont, e.source).out_edges, e.edge_id) ==
             2) {
        ++(e.edge_id);
      }
      if (adjlistBC_check_desc_validity(
              BC_desc_to_value(*p_cont, e.source).out_edges, e.edge_id) == 0) {
        return;
      }
      ...
      e.edge_id = BC_iterator_to_desc(
          BC_desc_to_value(*p_cont, e.source).out_edges,
          BC_get_begin_iter(BC_desc_to_value(*p_cont, e.source).out_edges));
    }
  }
  void decrement() {
    using SrcDesc = typename EDesc::source_descriptor;

    EDesc tmp_e =
        e;  // apply decrement to temporary descriptor, in case e is at the beginning.
    SrcDesc last_source = e.source;

    while (true) {
      while (adjlistBC_check_desc_validity(
          *p_cont, tmp_e.source)) {  // while at the end vertex.
      ...
      if (tmp_e.source != last_source) {  // if we moved back the vertex
        tmp_e.edge_id = BC_iterator_to_desc(
            BC_desc_to_value(*p_cont, tmp_e.source).out_edges,
            BC_get_end_iter(BC_desc_to_value(*p_cont, tmp_e.source).out_edges));
      }

      bool edge_range_was_empty = false;
      while (adjlistBC_check_desc_validity(
          BC_desc_to_value(*p_cont, tmp_e.source).out_edges, tmp_e.edge_id)) {
        // check if out-edge range is empty:
        if (adjlistBC_is_desc_at_begin(
                BC_desc_to_value(*p_cont, tmp_e.source).out_edges,
                tmp_e.edge_id)) {
          if (adjlistBC_is_desc_at_begin(*p_cont, tmp_e.source)) {
  ...
  const EDesc& dereference() const { return e; }
...
class adjlistBC_undir_eiter
    : public iterator_facade<adjlistBC_undir_eiter<EDesc, EIter>, EDesc,
                             std::bidirectional_iterator_tag, EDesc> {
 public:
  using self = adjlistBC_undir_eiter<EDesc, EIter>;

  explicit adjlistBC_undir_eiter(EIter aCurIt) : cur_it(std::move(aCurIt)) {}
  adjlistBC_undir_eiter() : adjlistBC_undir_eiter(EIter()) {}
