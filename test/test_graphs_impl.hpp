...
std::enable_if_t<is_vertex_list_graph<Graph>::value>
...
intint_check_graph_vertex_values(const Graph &g, std::size_t expected_count,
  ...
  EXPECT_EQ(vp_list.size(), expected_count);
  for (int& i : vp_list) {
    EXPECT_EQ(i, *(ref_values++));
  }
}
...
template <typename Graph>
std::enable_if_t<is_edge_list_graph<Graph>::value>
check_graph_edge_count(const Graph &g, std::size_t expected_count) {
  EXPECT_EQ(num_edges(g), expected_count);
}
...
template <typename Graph>
std::enable_if_t<is_bidirectional_graph<Graph>::value>
...
template <typename Graph>
std::enable_if_t<!is_bidirectional_graph<Graph>::value> check_graph_in_degree(
    const Graph & /*unused*/,
...
check_graph_in_edge_values(const Graph &g,
  ...
  InEdgeIter ei;
  InEdgeIter ei_end;
  EXPECT_NO_THROW(tie(ei, ei_end) = in_edges(v, g));

  std::vector<int> e_list;
  std::vector<int> vp_list;
  for (; ei != ei_end; ++ei) {
    if (is_undirected_graph<Graph>::type::value) {
      EXPECT_TRUE((g[*ei] == g[source(*ei, g)] * 1000 + g[target(*ei, g)]) ||
  ...
  for (int& i : e_list) {
    EXPECT_EQ(i, *(edge_values++));
  }
  std::sort(vp_list.begin(), vp_list.end());
  for (int& i : vp_list) {
    EXPECT_EQ(i, *(vertex_values++));
  }
}
...
template <typename Graph>
std::enable_if_t<is_incidence_graph<Graph>::value>
...
template <typename Graph>
std::enable_if_t<!is_incidence_graph<Graph>::value> check_graph_out_degree(
    const Graph & /*unused*/,
    typename graph_traits<Graph>::vertex_descriptor /*unused*/,
    std::size_t /*unused*/) {}
...
check_graph_out_edge_values(const Graph &g,
  ...
  OutEdgeIter ei;
  OutEdgeIter ei_end;
  EXPECT_NO_THROW(tie(ei, ei_end) = out_edges(u, g));
...
template <typename Graph>
std::enable_if_t<!is_incidence_graph<Graph>::value> check_graph_out_edge_values(
    const Graph & /*unused*/,
    typename graph_traits<Graph>::vertex_descriptor /*unused*/,
    std::size_t /*unused*/, const int * /*unused*/, const int * /*unused*/) {}

template <typename Graph>
std::enable_if_t<!is_undirected_graph<Graph>::value>
...
TYPED_TEST(IntIntBglMutableGraphTest, AllCases) {
  ...
  EXPECT_NO_THROW(v_root = add_vertex(1, g));
  ...
  for (int i = 0; i < 4; ++i) {
    EXPECT_NO_THROW(v_rc[i] = add_vertex(g));
    g[v_rc[i]] = vp_rc[i];
    EXPECT_EQ(g[v_rc[i]], vp_rc[i]);
    bool edge_added_success = false;
    EXPECT_NO_THROW(tie(e_rc[i], edge_added_success) =
                        add_edge(v_root, v_rc[i], g));
    EXPECT_TRUE(edge_added_success);
    g[e_rc[i]] = ep_rc[i];
    EXPECT_EQ(g[e_rc[i]], ep_rc[i]);
  }
  ...
  for (std::size_t i = 0; i < 4; ++i) {
    EXPECT_NO_THROW(v_rc1c[i] = add_vertex(vp_rc1c[i], g));
    EXPECT_EQ(g[v_rc1c[i]], vp_rc1c[i]);
    bool edge_added_success = false;
    EXPECT_NO_THROW(tie(e_rc1c[i], edge_added_success) =
                        add_edge(v_rc[0], v_rc1c[i], ep_rc1c[i], g));
    EXPECT_TRUE(edge_added_success);
    EXPECT_EQ(g[e_rc1c[i]], ep_rc1c[i]);
  }
  ...
  for (std::size_t i = 0; i < 4; ++i) {
    EXPECT_NO_THROW(v_rc2c[i] = add_vertex(std::move(vp_rc2c[i]), g));
    bool edge_added_success = false;
    EXPECT_NO_THROW(tie(e_rc2c[i], edge_added_success) =
  ...
  EXPECT_EQ(last_e_of_rc2.second, true);
