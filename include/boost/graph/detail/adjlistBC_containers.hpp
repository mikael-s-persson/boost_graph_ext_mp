...
struct adjlistBC_vertex_config {
  typedef adjlistBC_vertex_stored_type<VertexListS, OutEdgeListS, DirectedS,
...
struct adjlistBC_edge_stored_type {
  typedef adjlistBC_edge_stored_type<VertexListS, OutEdgeListS, DirectedS,
                                     VertexProperties, EdgeProperties>
      self;
  typedef adjlistBC_vertex_config<VertexListS, OutEdgeListS, DirectedS,
                                  VertexProperties, EdgeProperties>
      VConfig;
  ...
  adjlistBC_edge_stored_type(vertex_descriptor aTarget = VConfig::null_vertex())
      : target(aTarget), data(){};
...
struct adjlistBC_edge_config {
  typedef typename adjlistBC_vertex_config<
      VertexListS, OutEdgeListS, DirectedS, VertexProperties,
      EdgeProperties>::descriptor vertex_descriptor;
...
struct adjlistBC_vertex_stored_type {
  ...
  typedef adjlistBC_edge_config<VertexListS, OutEdgeListS, DirectedS,
                                VertexProperties, EdgeProperties>
      Config;
  ...
  adjlistBC_vertex_stored_type(const VertexProperties& aData)
      : data(aData), out_edges(), in_edges(){};
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
  adjlistBC_vertex_stored_type(VertexProperties&& aData)
...
struct adjlistBC_out_edges_factory<OutEdgeCont*> {
  ...
  static void destroy_all_out_edges(BC_pooled_vector<VertexValue>& vcont) {
    typedef
        typename BC_pooled_vector<VertexValue>::container_type VertexContainer;
    typedef typename VertexContainer::iterator Iter;
...
template <typename Container, typename EdgeDesc, typename VertexCont,
          typename VertexDesc>
void adjlistBC_erase_edge(Container& cont, EdgeDesc e, VertexCont&,
                          VertexDesc) {
...
void adjlistBC_erase_edges_to(::boost::container::list<ValueType>& cont,
                              VertexCont&, VertexDesc, VertexDesc v,
                              std::size_t& e_count) {
  typedef typename ::boost::container::list<ValueType>::iterator Iter;
...
void adjlistBC_erase_edges_to(Container& cont, VertexCont&, VertexDesc,
...
// for OutEdgeListS = poolS
template <typename ValueType, typename VertexCont, typename VertexDesc>
void adjlistBC_erase_edges_to(BC_pooled_vector<ValueType>& cont, VertexCont&,
                              VertexDesc, VertexDesc v, std::size_t& e_count) {
  typedef typename BC_pooled_vector<ValueType>::container_type Container;
  typedef typename Container::iterator Iter;
...
void adjlistBC_erase_vertex(Container& cont, VertexDesc v) {
  ...
  typedef adjlistBC_out_edges_factory<typename VertexValue::edge_container_ptr>
      OutEdgeFactory;
...
void adjlistBC_update_assoc_out_edges_impl(Container& cont,
  ...
  for (typename std::vector<ValueType>::iterator it = v_temp.begin();
       it != v_temp.end(); ++it)
    it->target = new_v_id;
...
void adjlistBC_erase_vertex(::boost::container::vector<ValueType>& cont,
  ...
  typedef ::boost::container::vector<ValueType> Container;
  typedef typename Container::iterator Iter;
  typedef adjlistBC_out_edges_factory<typename ValueType::edge_container_ptr>
      OutEdgeFactory;
...
struct adjlistBC_vertex_container {
 ...
 public:
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
  adjlistBC_vertex_container(adjlistBC_vertex_container&& rhs)
      : m_vertices(), m_num_edges(0) {
    swap(rhs);
  };
  ...
  // NOTE: This WORKS for ALL vertex container types.
  void clear() {
    typedef adjlistBC_out_edges_factory<
  ...
  // NOTE: This WORKS for ALL vertex container types.
  typedef adjlistBC_select_vertex_iterator<VertexListS, vertex_container>
      VIterSelect;
  ...
  typedef adjlistBC_select_out_edge_iterator<OutEdgeListS, edge_container,
                                             edge_descriptor>
      OEIterSelect;
  ...
  // NOTE: This WORKS for ALL vertex container types.
  typedef adjlistBC_edge_iterator<vertex_container, edge_descriptor>
      edge_iterator;
