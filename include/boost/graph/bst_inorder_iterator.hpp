...
enum bst_traversal_status { OnLeftBranch, OnMiddleBranch, OnRightBranch };
...
class bst_inorder_iterator {
 ...
 private:
  ...
  bst_inorder_iterator(tree_type* aTree, vertex_type aU,
                       graph::detail::bst_traversal_status aStatus)
      : m_tree(aTree), m_u(aU), m_status(aStatus){};

 public:
  vertex_type base() const { return m_u; };

  bst_inorder_iterator(tree_type* aTree, vertex_type aU)
      : m_tree(aTree), m_u(aU), m_status(graph::detail::OnRightBranch) {
  ...
  static self begin(tree_type* aTree) {
    if ((aTree) && (num_vertices(*aTree)))
      return self(
          aTree,
          graph::detail::bst_go_down_left(*aTree, get_root_vertex(*aTree)),
          graph::detail::OnLeftBranch);
    else
      return self(NULL, graph_traits<tree_type>::null_vertex(),
                  graph::detail::OnRightBranch);
  };
    ...
    switch (m_status) {
      case graph::detail::OnLeftBranch:
        // on a left-leaf, must move up to the parent as a middle-point
        {
          ...
          if (eis.first ==
              eis.second) {  // at the root, go to the end (root, rightbranch).
            m_status = graph::detail::OnRightBranch;
            return *this;
          };
      ...
      case graph::detail::OnMiddleBranch:
        // on a middle-point, must move down to the right once, and then left to the bottom.
        {
          if (out_degree(m_u, *m_tree) > 1) {
            ...
            std::pair<child_vertex_iter, child_vertex_iter> cur_children =
                child_vertices(m_u, *m_tree);
            ++(cur_children.first);
            m_u =
                graph::detail::bst_go_down_left(*m_tree, *(cur_children.first));
      ...
      case graph::detail::OnMiddleBranch:
        // on a middle-point or right-point, must move down to the left once (if exists), and then right to the bottom.
        {
          if (out_degree(m_u, *m_tree)) {
            ...
            std::pair<child_vertex_iter, child_vertex_iter> cur_children =
                child_vertices(m_u, *m_tree);
            m_u = graph::detail::bst_go_down_right(*m_tree,
                                                   *(cur_children.first));
            if (m_u == *(cur_children.first)) {
              if (out_degree(m_u, *m_tree) == 0)
                m_status = graph::detail::OnLeftBranch;
