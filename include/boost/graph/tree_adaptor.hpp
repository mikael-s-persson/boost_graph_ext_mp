...
typename graph_traits<Graph>::vertex_descriptor create_root(const VProp& vp,
                                                            Graph& g);
#else
/**
 * Creates a root for the tree (clears it if not empty), and assigns the given vertex-property to it.
...
template <typename Graph>
void record_vertex_prop_impl(typename graph_traits<Graph>::vertex_descriptor,
                             ignore_output_iter&, Graph&){
    /* do nothing */
};
...
template <typename Graph>
void record_edge_prop_impl(typename graph_traits<Graph>::edge_descriptor,
                           ignore_output_iter&, Graph&){
    /* do nothing */
};
...
template <typename Graph>
void record_in_edge_prop_impl(typename graph_traits<Graph>::vertex_descriptor,
                              ignore_output_iter&, Graph&){
    /* do nothing */
};
...
std::pair<VertexOIter, EdgeOIter> clear_children_impl(
    ...
    EdgeOIter eit_out, Graph& g, const DummyTag&) {
  typedef typename graph_traits<Graph>::out_edge_iterator EdgeIter;
  typedef typename graph_traits<Graph>::vertex_descriptor Vertex;
  std::queue<Vertex> v_queue;
  v_queue.push(u);
...
std::pair<VertexOIter, EdgeOIter> clear_children_impl(
    typename graph_traits<Graph>::vertex_descriptor u, VertexOIter vit_out,
    EdgeOIter eit_out, Graph& g, const std::random_access_iterator_tag&) {
...
#else
      v_tmp_queue.push(
          add_child_vertex(v_tmp, g[target(*ei, g)], g[*ei], g_tmp).first);
#endif
    };
  };
