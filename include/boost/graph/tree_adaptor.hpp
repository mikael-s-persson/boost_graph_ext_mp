...
typename graph_traits<Graph>::vertex_descriptor get_root_vertex(
    const Graph& g) {
...
child_vertices(typename graph_traits<Graph>::vertex_descriptor v,
               const Graph& g) {
 ...
 */
template <typename Graph>
typename graph_traits<Graph>::vertex_descriptor parent_vertex(
...
typename graph_traits<Graph>::vertex_descriptor create_root(VProp&& vp,
                                                            Graph& g);
...
std::pair<typename graph_traits<Graph>::vertex_descriptor,
          typename graph_traits<Graph>::edge_descriptor>
add_child_vertex(typename graph_traits<Graph>::vertex_descriptor u, VProp&& vp,
                 Graph& g) {
  typename graph_traits<Graph>::vertex_descriptor v =
      add_vertex(std::forward<VProp>(vp), g);
  typename graph_traits<Graph>::edge_descriptor e = add_edge(u, v, g).first;
  return std::make_pair(v, e);
}
...
add_child_vertex(const typename graph_traits<Graph>::vertex_descriptor& u,
  ...
  typename graph_traits<Graph>::edge_descriptor e =
      add_edge(u, v, std::forward<EProp>(ep), g).first;
  return std::make_pair(v, e);
}
...
template <typename Graph>
void record_vertex_prop_impl(
    typename graph_traits<Graph>::vertex_descriptor /*unused*/,
    ignore_output_iter& /*unused*/, Graph& /*unused*/) {
  /* do nothing */
}

template <typename Graph, typename VertexOIter>
void record_vertex_prop_impl(typename graph_traits<Graph>::vertex_descriptor u,
                             VertexOIter& vit_out, Graph& g) {
  *(vit_out++) = std::move(g[u]);
}

template <typename Graph>
void record_edge_prop_impl(
    typename graph_traits<Graph>::edge_descriptor /*unused*/,
    ignore_output_iter& /*unused*/, Graph& /*unused*/) {
  /* do nothing */
}

template <typename Graph, typename EdgeOIter>
void record_edge_prop_impl(typename graph_traits<Graph>::edge_descriptor e,
                           EdgeOIter& eit_out, Graph& g) {
  *(eit_out++) = std::move(g[e]);
}

template <typename Graph>
void record_in_edge_prop_impl(
    typename graph_traits<Graph>::vertex_descriptor /*unused*/,
    ignore_output_iter& /*unused*/, Graph& /*unused*/) {
  /* do nothing */
}
...
typename enable_if<
    ...
    void>::type
record_in_edge_prop_impl(typename graph_traits<Graph>::vertex_descriptor u,
                         EdgeOIter& eit_out, Graph& g) {
  /* depends on bidirectionality of the graph type: */
  using InEdgeIter = typename graph_traits<Graph>::in_edge_iterator;
  using EdgeProp = typename Graph::edge_bundled;
  InEdgeIter iei;
  InEdgeIter iei_end;
...
record_in_edge_prop_impl(typename graph_traits<Graph>::vertex_descriptor u,
  ...
  using EdgeIter = typename graph_traits<Graph>::out_edge_iterator;
  using EdgeProp = typename Graph::edge_bundled;
...
template <typename Graph, typename VertexOIter, typename EdgeOIter,
          typename DummyTag>
std::pair<VertexOIter, EdgeOIter> clear_children_impl(
    typename graph_traits<Graph>::vertex_descriptor u, VertexOIter vit_out,
    EdgeOIter eit_out, Graph& g, const DummyTag& /*unused*/) {
  using EdgeIter = typename graph_traits<Graph>::out_edge_iterator;
  using Vertex = typename graph_traits<Graph>::vertex_descriptor;
  std::queue<Vertex> v_queue;
  v_queue.push(u);
...
std::pair<VertexOIter, EdgeOIter> clear_children_impl(
    typename graph_traits<Graph>::vertex_descriptor u, VertexOIter vit_out,
    EdgeOIter eit_out, Graph& g,
    const std::random_access_iterator_tag& /*unused*/) {
  ...
  while (!v_ori_queue.empty()) {
    ...
    for (tie(ei, ei_end) = out_edges(v, g); ei != ei_end; ++ei) {
      ...
      v_tmp_queue.push(add_child_vertex(v_tmp, std::move(g[target(*ei, g)]),
                                        std::move(g[*ei]), g_tmp)
                           .first);
    }
  }
 ...
 */
template <typename Graph>
void clear_children(typename graph_traits<Graph>::vertex_descriptor u,
                    Graph& g) {
  using VIter = typename graph_traits<Graph>::vertex_iterator;
  using category = typename std::iterator_traits<VIter>::iterator_category;
  clear_children_impl(u, graph::detail::ignore_output_iter(),
                      graph::detail::ignore_output_iter(), g, category());
}
 ...
 */
template <typename Graph, typename OutputIter>
OutputIter clear_children(typename graph_traits<Graph>::vertex_descriptor u,
                          OutputIter it_out, Graph& g) {
  using VIter = typename graph_traits<Graph>::vertex_iterator;
  using category = typename std::iterator_traits<VIter>::iterator_category;
  return clear_children_impl(u, it_out, graph::detail::ignore_output_iter(), g,
                             category())
      .first;
}
...
void remove_branch(typename graph_traits<Graph>::vertex_descriptor u,
  ...
  using VIter = typename graph_traits<Graph>::vertex_iterator;
  using category = typename std::iterator_traits<VIter>::iterator_category;
  remove_branch_impl(u, graph::detail::ignore_output_iter(),
                     graph::detail::ignore_output_iter(), g, category());
}
 ...
 */
template <typename Graph, typename OutputIter>
OutputIter remove_branch(typename graph_traits<Graph>::vertex_descriptor u,
                         OutputIter it_out, Graph& g) {
  using VIter = typename graph_traits<Graph>::vertex_iterator;
  using category = typename std::iterator_traits<VIter>::iterator_category;
  return remove_branch_impl(u, it_out, graph::detail::ignore_output_iter(), g,
                            category())
      .first;
}
...
template <typename Graph>
typename vertex_bundle_type<Graph>::type& get_property(
    typename graph_traits<Graph>::vertex_descriptor v_i, Graph& g) {
  return g[v_i];
}

template <typename Graph>
const typename vertex_bundle_type<Graph>::type& get_property(
    typename graph_traits<Graph>::vertex_descriptor v_i, const Graph& g) {
  return g[v_i];
}

template <typename Graph>
typename edge_bundle_type<Graph>::type& get_property(
    typename graph_traits<Graph>::edge_descriptor e_i, Graph& g) {
  return g[e_i];
}

template <typename Graph>
const typename edge_bundle_type<Graph>::type& get_property(
    typename graph_traits<Graph>::edge_descriptor e_i, const Graph& g) {
  return g[e_i];
}
