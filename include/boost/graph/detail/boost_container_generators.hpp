...
struct BC_hole_desc {
  std::size_t value;
  explicit BC_hole_desc(
      std::size_t aValue = (std::numeric_limits<std::size_t>::max)())
      : value(aValue){};

  bool operator==(BC_hole_desc rhs) const {
...
bool BC_is_elem_valid(const ValueType&) {
  return true;
};
...
struct BC_edge_is_not_hole {
  const Container* p_cont;
  explicit BC_edge_is_not_hole(const Container* aPCont = NULL)
      : p_cont(aPCont){};
  template <typename Desc>
...
struct BC_undir_edge_desc : EdgeDesc {
  ...
  explicit BC_undir_edge_desc(const EdgeDesc& aBase = EdgeDesc(),
...
Iter BC_desc_to_iterator(const Container&, Iter it) {
  return it;
};
...
template <typename Container, typename Iter>
Iter BC_iterator_to_desc(const Container&, Iter it) {
  return it;
};
...
typename Container::value_type& BC_desc_to_value(
    const Container&, typename Container::iterator it) {
  return *it;
};
...
struct ignore_output_iter {
  ...
  ignore_output_iter& operator++() {
    return *this;
  };
  ignore_output_iter& operator++(int) {
    return *this;
  };
  ignore_output_iter& operator--() {
    return *this;
