...
struct ltreeBC_vertex_config {
  typedef ltreeBC_vertex_stored_type<VertexListS, OutEdgeListS, DirectedS,
...
struct ltreeBC_edge_stored_type {
  typedef ltreeBC_edge_stored_type<VertexListS, OutEdgeListS, DirectedS,
                                   VertexProperties, EdgeProperties>
      self;
  typedef ltreeBC_vertex_config<VertexListS, OutEdgeListS, DirectedS,
                                VertexProperties, EdgeProperties>
      VConfig;
  ...
  ltreeBC_edge_stored_type(vertex_descriptor aTarget = VConfig::null_vertex())
      : target(aTarget), data(){};
...
struct ltreeBC_edge_config {
  typedef typename ltreeBC_vertex_config<
...
struct ltreeBC_vertex_stored_type {
  ...
  typedef ltreeBC_edge_config<VertexListS, OutEdgeListS, DirectedS,
                              VertexProperties, EdgeProperties>
      Config;
  ...
  ltreeBC_vertex_stored_type(const VertexProperties& aData)
      : data(aData), out_edges(), in_edge(){};
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
  ltreeBC_vertex_stored_type(VertexProperties&& aData)
...
template <typename DirectedS, typename ValueType>
typename enable_if<is_same<DirectedS, directedS>, void>::type
ltreeBC_invalidate_in_edges(::boost::container::vector<ValueType>&,
                            std::size_t){};
...
void ltreeBC_erase_vertices(BC_pooled_vector<ValueType>& cont,
                            std::vector<std::size_t>& v_list) {
  for (typename std::vector<std::size_t>::iterator it = v_list.begin();
       it != v_list.end(); ++it)
    adjlistBC_erase_vertex(cont, *it);
};
...
#ifndef BOOST_NO_CXX11_RVALUE_REFERENCES
  ltreeBC_vertex_container(ltreeBC_vertex_container&& rhs)
      : m_vertices(), m_root(BC_null_desc<vertex_descriptor>::value()) {
    swap(rhs);
  };
 ...
 * *************************************************************************************/
  ...
  void add_root_vertex(const VertexProperties& vp) {
    m_root = add_new_vertex(vp);
  };
#else
  ...
  std::pair<vertex_descriptor, edge_descriptor> add_child(
      vertex_descriptor v, const VertexProperties& vp,
      const EdgeProperties& ep) {
